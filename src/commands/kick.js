...
module.exports = {
    ...
    async execute(msg, args) {
        ...
        if (!mentionMember) {
            await safeDelete(msg);
            await sendTemporaryMessage(msg.channel, "You need to give me a user to kick!");
            return;
        }
        ...
        if (targetHighestRole.position >= requesterHighestRole.position) {
            ...
            await sendTemporaryMessage(
                msg.channel,
                `${mentionMember.user.username} is above you. You may only kick users beneath your user group.`
            );
        ...
        if (!mentionMember.kickable) {
            ...
            await sendTemporaryMessage(
                msg.channel,
                "Woah! That user is far too strong for me to kick!"
            );
        ...
        try {
            ...
            console.error("[KICK COMMAND] Failed to kick user:", err);
            await sendTemporaryMessage(msg.channel, "I tried to kick that user, but something went wrong.");
            return;
        }
        ...
        await logCommandUsage(msg, `${prefix}kick`);
    }
};